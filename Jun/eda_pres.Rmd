...
maternal_clean_map <- maternal_clean |>
  ...
  filter(TobaccoUse_bin == 0, Diabetes_bin == 0, Hypertension_bin == 0)
...
fig <- fig %>% add_trace(
    ...
    zmin = min(maternal_clean_map$AveragePrePregnancyBMI, na.rm = TRUE),
    zmax = max(maternal_clean_map$AveragePrePregnancyBMI, na.rm = TRUE)
  )
fig <- fig %>% colorbar(title = "Pre-Pregnancy BMI")
fig <- fig %>% layout(
    title = 'Pre-Pregnancy BMI (No Tobacco, Hypertension, Diabetes)',
    geo = g
  )
