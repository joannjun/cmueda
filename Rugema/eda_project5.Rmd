...
| Step | Description |
|------|-------------|
| 1️⃣ Data Cleaning & Transformation | Handle missing values, encode categorical variables, remove `"Unknown"` entries |
| 2️⃣ Exploratory Data Analysis | Create bar charts and scatter plots to explore relationships between risk factors and outcomes |
| 3️⃣ Correlation Analysis | Generate a heatmap and perform pairwise scatter plots to identify strong numeric relationships |
| 4️⃣ Dimensionality Reduction | Conduct PCA and examine explained variance and scree plots |
| 5️⃣ Clustering | Perform K-means clustering on the first 4 principal components and determine optimal `k` |
| 6️⃣ Geographic Visualization | Map cluster assignments to states to identify regional patterns |
...
```{r}
maternal_with_cluster <- maternal |>
  bind_cols(cluster = my_kmeans$cluster)

state_clusters <- maternal_with_cluster |>
  group_by(State, cluster) |>
  summarise(count = n(), .groups = 'drop') |>
  group_by(State) |> 
  slice_max(order_by = count, n = 1)

# Make sure 'state' is lowercase and 'cluster' is a factor
state_clusters <- state_clusters |>
  rename(state = State) |>
  mutate(cluster = as.factor(cluster))

plot_usmap(data = state_clusters, 
           values = 'cluster', 
           regions = 'states') +
  scale_fill_manual(values = c("1" = "red",
                               ...
                               "4" = "purple"),
                    name = "cluster") +
  labs(title = "U.S. States by Maternal Risk Profile Cluster") +
  theme_minimal()
```
